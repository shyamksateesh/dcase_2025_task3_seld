...
class SELDLossADPIT(nn.Module):
    ...
    def forward(self, output, target):
        ...
        Args:
            output:
                audio:        (batch_size, n_frames=50, 117) -> 117 = 3 (tracks) x 3 (act*x, act*y, dist) x 13 (classes)
            target:
                audio:        (batch_size, n_frames=50, 6, 4, 13) -> 6 (dummy tracks), 4 (act, x, y, dist), 13 (classes)
        Return:
            loss: scalar
        """
        ...
        # if use_vtm: output_accdoa = self._apply_vtm_mask(output_accdoa)
        ...
        for i in range(num_permutation):
            loss_sum += (list_loss_accdoa[i]) * (loss_accdoa_min == i)
        loss = loss_sum.mean()
        return loss
